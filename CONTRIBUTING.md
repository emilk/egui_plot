...
- Each crate under `examples/` has a `screenshot_matches_snapshot` test generated by the `examples_utils::make_main!` macro.
- Update a screenshot locally with `UPDATE_SNAPSHOTS=1 cargo test -p <example_name> screenshot_matches_snapshot`.
