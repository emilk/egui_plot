...
/// Macro to generate common builder methods for plot items.
#[macro_export]
macro_rules! builder_methods_for_base {
    () => {
        ...
        ///
        /// Setting the name via this method does not change the item's id, so you can
        /// use it to change the name dynamically between frames without losing the
        /// item's state. You should make sure the name passed to [`Self::new`] is
        /// unique and stable for each item, or set unique and stable ids explicitly via
        /// [`Self::id`].
        #[inline]
        ...
        ///
        /// By default the id is determined from the name passed to [`Self::new`], but
        /// it can be explicitly set to a different value.
        #[inline]
