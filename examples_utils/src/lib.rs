...
pub mod internal {
    ...
    pub fn run_screenshot_test<State>(
        builder: impl FnOnce(&mut eframe::CreationContext<'_>) -> State,
        manifest_dir: &str,
    ) where
        State: eframe::App,
    {
        let snapshot_name = "screenshot";
        ...
        let options = SnapshotOptions::new().threshold(1.0).output_path(output_path.clone());

        harness.snapshot_options(snapshot_name, &options);

        // Generate thumbnail
        let screenshot_path = output_path.join(format!("{snapshot_name}.png"));
        if let Ok(img) = image::open(&screenshot_path) {
            let thumbnail = img.resize_exact(192, 192, image::imageops::FilterType::Lanczos3);
            let thumbnail_path = output_path.join(format!("{snapshot_name}_thumb.png"));
            if let Err(e) = thumbnail.save(&thumbnail_path) {
                eprintln!("Failed to save thumbnail to {}: {}", thumbnail_path.display(), e);
            }
        } else {
            eprintln!(
                "Failed to load screenshot from {} for thumbnail generation",
