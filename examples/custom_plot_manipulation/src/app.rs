...
impl CustomPlotManipulationExample {
    ...
    pub fn ui(&mut self, ui: &mut egui::Ui) -> Response {
        ...
        let (scroll, pointer_down, modifiers) = ctx.input(|i| {
        ...
        // Left side panel with the options
        egui::SidePanel::left("custom_plot_manipulation_controls")
            .resizable(true)
            .exact_width(220.0)
            .show(&ctx, |ui| {
                ui.heading("Options");
                ui.separator();
                ui.checkbox(&mut self.lock_x, "Lock x axis").on_hover_text(
                    "Check to keep the X axis fixed, i.e., pan and zoom will only affect the Y axis",
                );
                ui.checkbox(&mut self.lock_y, "Lock y axis").on_hover_text(
                    "Check to keep the Y axis fixed, i.e., pan and zoom will only affect the X axis",
                );
